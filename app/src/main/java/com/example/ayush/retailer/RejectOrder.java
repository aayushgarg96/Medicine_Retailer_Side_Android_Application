 ...
 */
public class RejectOrder extends AsyncTask<String, Void, String> {

    Context C;
    String fresult;

    public RejectOrder(Context C) {
        this.C = C;
    }
    ...
    protected String doInBackground(String... params) {
        String k = params[0];
        try {
            URL url = new URL("http://minorprojectf5.esy.es/rejectOrder.php");
    ...
    protected void onPostExecute(String s) {
        if(s.equals("YES")){
            Toast.makeText(C, "Order Rejected.", Toast.LENGTH_SHORT).show();
        }
        else{
